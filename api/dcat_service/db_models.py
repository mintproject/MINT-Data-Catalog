...
class DatasetDB(Base):
    ...
    id = Column(postgresql.UUID(as_uuid=True), primary_key=True)
    provenance_id = Column(postgresql.UUID(as_uuid=True), ForeignKey("provenance.id"), index=True, nullable=False)
...
resource_variables = Table("resources_variables", Base.metadata,
                               Column("resource_id", postgresql.UUID(as_uuid=True), ForeignKey("resources.id", ondelete='CASCADE'), primary_key=True),
...
class ResourceDB(Base):
    ...
    id = Column(postgresql.UUID(as_uuid=True), primary_key=True)
    dataset_id = Column(postgresql.UUID(as_uuid=True), ForeignKey("datasets.id"), index=True, nullable=False)
    provenance_id = Column(postgresql.UUID(as_uuid=True), ForeignKey("provenance.id"), index=True, nullable=False)
    ...
    variables = relationship("VariableDB", secondary="resources_variables", back_populates="resources")
...
class VariableDB(Base):
    ...
    id = Column(postgresql.UUID(as_uuid=True), primary_key=True)
    dataset_id = Column(postgresql.UUID(as_uuid=True), ForeignKey("datasets.id", ondelete='CASCADE'), index=True, nullable=False)
    name = Column(String, nullable=False)
    # spatial_coverage = Column(Geometry(srid=LOCATION_SRID))
    # start_time = Column(DateTime, index=True)
    # end_time = Column(DateTime, index=True)
    dataset = relationship("DatasetDB", back_populates='variables')
    resources = relationship("ResourceDB", secondary="resources_variables", back_populates="variables")
    standard_variables = relationship("StandardVariableDB", secondary="variables_standard_variables", back_populates="variables")
    json_metadata = Column(JSONB, nullable=False)

    __table_args__ = (UniqueConstraint('dataset_id', 'name', name="variables_dataset_id_name_unique_constraint"), )
...
class StandardVariableDB(Base):
    ...
    variables = relationship("VariableDB", secondary="variables_standard_variables", back_populates="standard_variables")
...
class SpatialCoverageIndexDB(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint(indexed_type, indexed_id, name="indexed_spatial_coverage_constraint"), )
...
class TemporalCoverageIndexDB(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint(indexed_type, indexed_id, name="indexed_temporal_coverage_constraint"), )
...
class EventDB(Base):
    ...
    id = Column(postgresql.UUID(as_uuid=True), primary_key=True)
    timestamp = Column(DateTime(timezone=False), nullable=False, index=True, server_default=func.now())
